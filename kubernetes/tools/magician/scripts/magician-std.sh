...
if [ -z $TEMPLATE_PATH ]; then
  TEMPLATE_PATH=./template/std/config
fi

if [ -z $SCRIPTS_PATH ]; then
    SCRIPTS_PATH=./template/std/scripts
fi
...
function generateSiteConfig {
  echo "Generating configs..."
    ...
    filelist="deployment.yaml service.yaml ingress.yaml"
    for f in $filelist; 
      do
...
function handleDeploymentSecrets {
   echo "Handling secrets..."
    ...
    local target_deployment_file=$OUTPUT_PATH/config/${app_name}-deployment.yaml
...
function hello {
  echo "Creating a magician standard app 🎩..."
}
