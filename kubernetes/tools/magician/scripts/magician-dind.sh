...
if [ -z $TEMPLATE_PATH ]; then
  TEMPLATE_PATH=./template/dind/config
fi

if [ -z $SCRIPTS_PATH ]; then
    SCRIPTS_PATH=./template/dind/scripts
fi
...
function handleDeployment {
  ...
  replaceOccurence __K8_APP_DNS__ $app_dns $deployment_file
}
...
function hello {
  echo "Creating a magician dind app 🎩..."
}
